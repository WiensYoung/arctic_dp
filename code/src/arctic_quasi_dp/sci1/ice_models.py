...
class EmpiricalIceLoadModel(IceLoadModel):
    ...
    Parameters:
    - crushing_strength: default 0.0003 MPa (proxy-scale, ~1 kN ice forces).
      For full-scale first-year ice, use 1-5 MPa per ISO 19906.
    - structure_factor: 0.45 (Lindqvist 1989)
    - waterline_angle: 30 deg (typical ice-breaking bow)

    Literature parameters are traceable to published sources.
    ...
    def compute(
        ...
        from .ice_force_common import compute_ice_force_ned, compute_ice_force_body
        force_ned = compute_ice_force_ned(
            concentration, thickness, drift_speed, np.degrees(float(drift_direction)),
            self.crushing_strength_mpa, vessel_beam, self.structure_factor,
            math.radians(self.waterline_angle_deg),
        )
        force_body = compute_ice_force_body(
            concentration, thickness, drift_speed, np.degrees(float(drift_direction)),
            psi, self.crushing_strength_mpa, vessel_beam, vessel_length,
            self.structure_factor, math.radians(self.waterline_angle_deg),
        )
        from .ice_force_common import lindqvist_speed_factor, lindqvist_angle_factor
        c = float(np.clip(concentration, 0.0, 1.0))
        h = max(0.0, float(thickness))
        v = max(0.0, float(drift_speed))
        base_force = (
            self.crushing_strength_mpa * 1e6 * h * vessel_beam * self.structure_factor
            * lindqvist_speed_factor(v) * lindqvist_angle_factor(math.radians(self.waterline_angle_deg)) * c
        )
...
class BenchmarkIceLoadModel(IceLoadModel):
    ...
    def __init__(self, reference_force_n: float = 100.0):
        """Initialize with a reference force magnitude.

        Default 100 N — a conservative proxy-scale reference value.
        """
    ...
    def compute(
        ...
        # Scale reference force by ice parameters (0.01 offset avoids zero-div/near-zero in scale)
        scale = c * (0.01 + h) * (0.01 + v)
