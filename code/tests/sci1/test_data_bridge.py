...
# 优先使用真实 Copernicus 数据 (231 MB, 含冰漂移)
# data/ 在项目根目录 (code/ 的上级)
_PROJECT_ROOT = Path(__file__).resolve().parents[2].parent
COPERNICUS_FILE = _PROJECT_ROOT / "data/sci1_sources/copernicus/arctic_ice_2024_jan1_7_real.nc"
ERA5_WIND_FILE = _PROJECT_ROOT / "data/sci1_sources/era5/era5_arctic_wind_2020_jan1_7_real.nc"
...
class TestDataDrivenIceSchedule:
    ...
    def test_missing_file_returns_zero(self):
        ...
        s = schedule.at(0.0)
        # T2 fix: check ALL fields, not just concentration
        assert s.concentration == 0.0
        assert s.thickness == 0.0
        assert s.drift_speed == 0.0
        assert s.drift_direction == 0.0


class TestExtractIceStatistics:
    @pytest.mark.skipif(not HAS_COPERNICUS, reason="Copernicus data not downloaded")
    def test_extract_stats(self):
        stats = extract_ice_statistics(COPERNICUS_FILE, lat_range=(75, 85))
