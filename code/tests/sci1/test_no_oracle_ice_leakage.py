...
class TestNoOracleIceLeakage:
    ...
    def test_supervisor_risk_uses_estimated_ice(self):
        ...
        # T2 fix: verify risk is computed from estimated ice, not raw ice
        # The ice_aware observer should have modified the estimate from raw values
        ice_est = ctrl.ice_aware._ice_est
        raw_ice = ctrl._raw_ice

        assert isinstance(risk, float)
        assert 0.0 <= risk <= 1.0
        # The risk should reflect the estimated ice state
        # With observer initialization, _ice_est should have been populated
        assert "concentration" in ice_est, "ice_est should have concentration key"
