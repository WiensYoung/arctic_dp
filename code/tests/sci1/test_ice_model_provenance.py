...
class TestIceModelProvenance:
    ...
    def test_stochastic_varies_with_rng(self):
        model = StochasticIceLoadModel()
        # 需要 concentration > 0 才能产生随机扰动
        r1 = model.compute(0.7, 0.5, 0.8, 0.3, 0.0, 100.0, 20.0, rng=np.random.default_rng(1))
        r2 = model.compute(0.7, 0.5, 0.8, 0.3, 0.0, 100.0, 20.0, rng=np.random.default_rng(2))
        # 不同种子应产生不同结果 (概率极高)
        assert not np.allclose(r1.force_body, r2.force_body, rtol=0.01)
