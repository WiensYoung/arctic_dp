...
class TestThrusterMetrics:
    ...
    def test_allocation_residual(self):
        ...
        thrusts, feasible = alloc.allocate(tau_desired)
        tau_actual = alloc.resulting_tau(thrusts)
        residual = np.linalg.norm(tau_desired - tau_actual)
        # T2 fix: meaningful assertion — residual should be small for feasible allocation
        if feasible:
            assert residual < np.linalg.norm(tau_desired) * 0.5, (
                f"Allocation residual too large: {residual:.1f} vs desired norm {np.linalg.norm(tau_desired):.1f}"
            )
        else:
            # Infeasible allocation may have larger residual but still finite
            assert np.isfinite(residual), "Residual should be finite even for infeasible allocation"
