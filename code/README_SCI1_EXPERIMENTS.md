...
9. **统计显著性检验**: paired Wilcoxon signed-rank + paired Cohen's dz + bootstrap CI + Holm correction
...
- 表 4：统计显著性检验 (paired Wilcoxon signed-rank p 值、paired Cohen's dz、Holm-corrected p)。
- 图 1：分层 DP 增强架构。
...
## 方法层补丁：CVaR-adaptive Soft-HOCBF-QP safety filter

本轮方法层新增 `src/arctic_quasi_dp/sci1/control/`，通过 wrapper 方式把 nominal controller 输出的 `tau_des` 投影为安全约束后的 `tau_safe`，并在 `sim_loop` trace 中保留 `tau_des/tau_safe/tau_actual` 三路审计字段。该层当前定位为 **proxy-scale benchmark 方法层**，不是 full-scale DP3 船舶安全证明。

### 可用控制器

| 控制器 | 说明 |
|--------|------|
| `no_safety_filter` | 关闭 safety filter，用于 nominal 等价对照 |
| `fixed_soft_hocbf` | 固定 alpha 的 Soft-HOCBF-QP；当前默认使用 proxy actuator-aware inner polygon 约束 |
| `cvar_soft_hocbf` | 使用 CVaR-style tail-risk proxy 调制 alpha1/alpha2 的 Soft-HOCBF-QP；当前默认使用 proxy actuator-aware inner polygon 约束 |

### 运行方法层 smoke

```bash
arctic-sci1 --config configs/sci1/sci1_method_smoke.yaml
```

或在测试环境中先跑最小验证：

```bash
pytest -q tests/sci1/test_safety_filter_result.py tests/sci1/test_method_integration.py
```

### QP 求解器说明

`SoftHOCBFSafetyFilter` 会优先使用 OSQP；如果当前 Python 环境未安装 `osqp`，会自动使用明确标记为 `scipy_fallback` 的 SciPy/SLSQP fallback 求解同一个 soft-HOCBF QP。fallback 会解析消去非负 slack：

```text
delta(tau) = max(0, b_hocbf - a_hocbf @ tau)
```

因此即使缺少 OSQP，基础测试仍可运行；但正式 method/paper 结果应安装 `osqp` 并在 trace 中确认 `safety_filter_solver_backend=osqp`，否则不能宣称 OSQP-based real-time performance。

### Honest limitations / 不得夸大

当前结果可以支持：

- proxy-scale benchmark 中的 Soft-HOCBF-QP safety filter；
- proxy actuator-aware inner polygon feasible-set 约束（用于方法实验，不能解释为全尺度分配可行性证明）；
- CVaR-style tail-risk proxy 对 HOCBF alpha1/alpha2 的自适应调制；
- `tau_des/tau_safe/tau_actual` 三路审计 trace；
- chattering / actuator dynamics 指标；
- solver backend trace 区分 `osqp` 与 `scipy_fallback`。

当前结果不能支持：

- full allocation-aware proof for real vessels；当前 polygon 是 proxy-scale conservative approximation；
- full-scale DP3 validation；
- 真实北极航行安全性的形式化证明；
- formal HOCBF invariance guarantee；当前是 proxy-scale approximation；
- true CVaR-constrained stochastic control；当前是 proxy estimator；
- Tube-MPC / ADRC baseline comparison；当前尚未实现。

## Method-scope note

For the exact scope of the Soft-HOCBF-QP implementation, CVaR-style risk
modulation, actuator-aware polygon approximation, and unsupported claims, see
`docs/METHOD_HOCBF_LIMITATIONS.md`. The current repository remains a proxy-scale
benchmark/method artifact and must not be described as full-scale DP3 validation
or a formal proof of real Arctic ship safety.

## CPU-parallel execution on a 32-core Linux server

For large method or paper profiles, use task-level process parallelism over
`scenario x controller x seed`.  This is the most effective way to use a
32-core AMD EPYC node because each simulation run is independent.
...
The first command runs simulations and writes one atomic result file per task
under `raw/tasks/`.  The second command recomputes statistical comparison
tables from the merged per-seed metrics.  Use `sci1_representative_traces.yaml`
for a small trace-producing run after the summary-only paper run.
